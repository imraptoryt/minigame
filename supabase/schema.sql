...
-- ============================================================================
-- SEED: admin account — username "raptor" / password "admin"
-- Change this password on first login (profile menu ▸ Mot de passe)!
--
-- This writes directly into Supabase's internal auth schema, which is a
-- well-known pattern for seeding a ready-to-use account (no email required),
-- but it is technically best-effort: if your Supabase Postgres version has
-- a slightly different auth.users/auth.identities shape, this block will
-- fail SAFELY (caught below) without breaking anything created above it.
-- If that happens, just sign up normally in the app with username "raptor" /
-- password "admin" — being the very first account, it becomes Patron
-- automatically and ends up identical.
-- ============================================================================
do $$
declare
  v_uid        uuid;
  ...
  v_role_id    uuid;
begin
  if exists (select 1 from auth.users where email = 'raptor@lsc.internal') then
    raise notice 'Admin "raptor" already exists — skipping seed.';
    return;
  end if;
  ...
  insert into auth.users (
    ...
    'raptor@lsc.internal', crypt('admin', gen_salt('bf')), now(),
  ...
  insert into auth.identities (
    ...
    jsonb_build_object('sub', v_uid::text, 'email', 'raptor@lsc.internal'),
    'email', now(), now(), now()
  );
  ...
  insert into profiles (id, company_id, full_name, employee_number, username, char_id, phone, bank_number)
  values (v_uid, v_company_id, 'Raptor', floor(random() * 900000 + 100000)::int, 'raptor', 'ADMIN', '555-0100', '100001');

  select id into v_role_id from roles where company_id = v_company_id and name = 'Patron' limit 1;
  if v_role_id is null then
    insert into roles (company_id, name, color, is_base, priority)
      values (v_company_id, 'Patron', '#16A34A', true, 100) returning id into v_role_id;
    insert into role_permissions (role_id, permission_key) select v_role_id, key from permissions;
  end if;
  insert into user_roles (user_id, role_id) values (v_uid, v_role_id) on conflict do nothing;

  insert into employees (company_id, profile_id, full_name, grade, status)
  values (v_company_id, v_uid, 'Raptor', 'Patron', 'active');

  if not exists (select 1 from product_categories where company_id = v_company_id) then
    perform seed_starter_catalogue(v_company_id);
  end if;

exception when others then
  raise notice 'Admin seed skipped — could not write to the auth schema on this Supabase instance (%). Just sign up normally in-app with username "raptor" / password "admin" instead.', sqlerrm;
end $$;

-- ============================================================================
-- SEED: second admin account — username "Raptor2" / password "admin2"
-- Same pattern as above (see its comments), kept independent so it isn't
-- affected by whatever state the first "raptor" account ended up in.
-- ============================================================================
do $$
declare
  v_uid        uuid;
  ...
  v_role_id    uuid;
begin
  if exists (select 1 from auth.users where email = 'raptor2@lsc.internal') then
    raise notice 'Admin "Raptor2" already exists — skipping seed.';
    return;
  end if;
  ...
  insert into auth.users (
    ...
    '00000000-0000-0000-0000-000000000000', v_uid, 'authenticated', 'authenticated',
    'raptor2@lsc.internal', crypt('admin2', gen_salt('bf')), now(),
    '{"provider":"email","providers":["email"]}'::jsonb, '{}'::jsonb, now(), now(), '', ''
  );
  ...
  insert into auth.identities (
    ...
    gen_random_uuid(), v_uid, v_uid::text,
    jsonb_build_object('sub', v_uid::text, 'email', 'raptor2@lsc.internal'),
    'email', now(), now(), now()
  );
  ...
  insert into profiles (id, company_id, full_name, employee_number, username, char_id, phone, bank_number)
  values (v_uid, v_company_id, 'Raptor2', floor(random() * 900000 + 100000)::int, 'Raptor2', 'ADMIN2', '555-0200', '100002');
  ...
  insert into employees (company_id, profile_id, full_name, grade, status)
  values (v_company_id, v_uid, 'Raptor2', 'Patron', 'active');
...
exception when others then
  raise notice 'Admin seed skipped — could not write to the auth schema on this Supabase instance (%). Just sign up normally in-app with username "Raptor2" / password "admin2" instead.', sqlerrm;
end $$;

-- ============================================================================
-- NOTE ON GETTING STARTED
--   1. In Supabase, go to Authentication ▸ Settings and turn OFF "Confirm
--      email" (Enable email confirmations). Accounts here never use a real
--      email address, so a confirmation link could never be delivered.
--   2. Run this whole file once in the Supabase SQL editor.
--   3. Deploy the site. Log in immediately with username "raptor" / password
--      "admin", or username "Raptor2" / password "admin2" (both Patron,
--      every permission) — change the password on first login. Or sign up
--      your own account: the very first one ever created becomes Patron
--      automatically and seeds the starter catalogue (Services / Customs /
--      Ventes / Peinture).
